...
You can run the ROCm-RAG extraction and retrieval pipelines in a Docker container in an interactive session to explore and debug the container manually. You can scrape and extract data to the `Weaviate <https://docs.weaviate.io/weaviate>`__ vector database (the AI-native vector database used by ROCm-RAG), then retrieve that data with the RAG retrieval pipeline.
...
2. Create a self-signed certificate with the SAN included.
...
6. Test and reload the nginx configuration.
