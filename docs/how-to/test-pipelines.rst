...
2. Enable user sign-ups.
   ...
   Select **Enable User Sign Ups** to allow new user registration. User data saves to ``/rag-workspace/rocm-rag/external/open-webui/backend/data/webui.db``:

   .. image:: ../images/admin_general.png
      :align: center
      :alt: Admin general settings

3. Add the APIs to the RAG server.

   By default, Haystack is running on port ``1416`` and LangGraph is running on port ``20000``:

   .. image:: ../images/admin_add_new_connection_rag.png
      :align: center
      :alt: Add new RAG connection

4. Add APIs to the example LLM server.

   If ``ROCM_RAG_USE_EXAMPLE_LLM=True``, use the default API URL. Otherwise, replace this API URL with your inference server URL:

   .. image:: ../images/admin_add_new_connection_llm.png
      :align: center
      :alt: Add new LLM connection
...
5. Configure audio settings.
   ...
   Select which fast whisper model (here's a `list of models provided by fast-whisper <https://github.com/SYSTRAN/faster-whisper/blob/d3bfd0a305eb9d97c08047c82149c1998cc90fcb/faster_whisper/utils.py#L12>`__) and which TTS voice to use:   
...
.. image:: ../images/direct_llm_response.png
   :align: center
   :alt: Direct LLM response example
...
.. image:: ../images/rag_response.png
   :align: center
   :alt: RAG-enhanced response example
...
.. image:: ../images/rocm_ds_blog.png
   :align: center
   :alt: ROCm-DS blog page verification
